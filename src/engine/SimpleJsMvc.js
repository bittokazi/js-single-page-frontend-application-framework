...
export default class SimpleJsMvc {
  ...
  getRouteProps(path, currentPath) {
    ...
    path.split("/").forEach(element => {
      let allProps = element.match(/\{([^}]+)\}/g);
      if (allProps) {
        data[allProps[0].replace("{", "").replace("}", "")] = currentPath.split(
          "/"
        )[index];
      }
      index++;
    ...
    if (fromSystem || window.location.pathname != arr) {
      routeList.forEach(element => {
        if (this.matchRoute(element.path, arr)) {
          ...
          let reg = /function ([^\(]*)/;
          window["_self"] = element.controller(data);
          ...
          document.getElementById(
            "application"
          ).innerHTML = currentController.view();
  ...
  static renderView() {
    document.getElementById("application").innerHTML = currentController.view();
  }

  static renderComponent(data, component, content = ``) {
    let componentInstance = component(data, content);
